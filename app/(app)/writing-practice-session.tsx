...
export default function WritingPracticeSessionScreen() {
  ...
  const restoreSavedWritingPracticeSession = useCallback(async (): Promise<boolean> => {
    ...
    if (savedSession.mode === "freehand") {
      router.replace({
        pathname: "/writing-practice-freehand-session",
        params: { resume: "true" },
      });
      return true;
    }
  ...
  }, [candidatePool, config, currentIndex, isComplete, questions, results]);
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      if (params.sessionId) {
        ...
        if (configData) {
          const parsedConfig = JSON.parse(configData) as WritingPracticeConfig;
          setConfig({
            ...parsedConfig,
        ...
        <View style={styles.promptContainer}>
          ...
          {currentQuestion.readings.length > 0 && (
            <>
              ...
              <Text style={[styles.promptReading, { color: theme.textSecondary }]}>
