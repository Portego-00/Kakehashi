...
export default function KanaKanjiConfigScreen() {
  const { theme } = useTheme();
  const { userData } = useAuthStore();
  const { autoSwitchKeyboard, setAutoSwitchKeyboard } = useSettingsStore();
  ...
  const canStart = () => {
    ...
    return hasAnySrs && autoSwitchKeyboard;
  };
    ...
    if (!canStart()) {
      if (!autoSwitchKeyboard) {
        Alert.alert(
          ...
          "Enable \"Switch to Japanese Keyboard\" to start Kana to Kanji mode.",
        );
      }
      return;
    }

    await clearExtraStudySessionState(EXTRA_STUDY_SESSION_STORAGE_KEYS.KANA_KANJI);

    try {
      const sessionId = `k2k_${Date.now()}`;
      await AsyncStorage.setItem(
        `kana_kanji_config_${sessionId}`,
        JSON.stringify(config),
      );

      router.push({
        pathname: "/kana-kanji-session",
        params: { sessionId },
      });
    } catch (error) {
      console.error("Failed to save kana->kanji config:", error);
      router.push({
        pathname: "/kana-kanji-session",
        params: {
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      const stored = await loadExtraStudyConfig<Config>(
        EXTRA_STUDY_CONFIG_STORAGE_KEYS.KANA_KANJI,
      );
      if (!isMounted) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const checkForSavedSession = async () => {
      const hasSavedSession = await hasExtraStudySessionState(
        EXTRA_STUDY_SESSION_STORAGE_KEYS.KANA_KANJI,
      );
      if (!hasSavedSession || !isMounted) {
        return;
      }

      Alert.alert("Resume Kana to Kanji Test?", "You have a test in progress.", [
  ...
  useEffect(() => {
    ...
    saveExtraStudyConfig(EXTRA_STUDY_CONFIG_STORAGE_KEYS.KANA_KANJI, config);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundColor }]}> 
      ...
      <View style={[styles.header, { backgroundColor: theme.backgroundColor }]}> 
        ...
        <Text style={[styles.headerTitle, { color: theme.textColor }]}>Kana to Kanji</Text>
      </View>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    setConfig((prev) => ({
                      ...
                      srsGroups: {
                        ...prev.srsGroups,
                        [key]: !prev.srsGroups[key as keyof typeof prev.srsGroups],
                      },
                    }))
                  }
                  activeOpacity={0.7}
        ...
        <View style={[styles.section, { backgroundColor: theme.cardBackground }]}> 
          <Text style={[styles.sectionTitle, { color: theme.textColor }]}>Input Requirements</Text>
          <Text style={[styles.sectionDescription, { color: theme.textSecondary, marginBottom: 10 }]}>This mode requires Japanese keyboard input so you can type kanji answers.</Text>
          ...
          <View
            style={[
              styles.toggleRow,
              {
                ...
                backgroundColor: autoSwitchKeyboard ? `${theme.primary}15` : "transparent",
                marginBottom: 6,
              },
            ]}
          >
        ...
        <View style={[styles.section, { backgroundColor: theme.cardBackground }]}> 
          ...
          <View
            style={[
              styles.toggleRow,
              {
                borderColor: config.useCustomLevelRange ? theme.secondary : theme.border,
        ...
        {!autoSwitchKeyboard && (
          ...
          >
            ...
            <Text style={[styles.warningText, { color: "#ff9800" }]}>Enable Japanese keyboard input to start Kana to Kanji mode.</Text>
          </View>
        )}
      ...
      >
        ...
        >
          ...
          <Text style={styles.startButtonText}>Start Kana to Kanji Test</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
