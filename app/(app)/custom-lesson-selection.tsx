...
export default function CustomLessonSelectionScreen() {
  ...
  const loadAvailableLists = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load subject lists for custom lessons:", error);
      setAvailableLists([]);
    } finally {
  ...
  const loadAllSubjects = useCallback(async () => {
    ...
    try {
      console.log("Loading all subjects for custom lesson selection...");
  ...
  }, [allSubjects, apiToken, loadAllSubjects]);
  ...
  const selectedListSubjectCount = useMemo(() => {
    ...
    availableLists.forEach((list) => {
      ...
      list.subjectIds.forEach((subjectId) => uniqueSubjectIds.add(subjectId));
    });
  ...
  return (
    ...
    >
      ...
      <View style={styles.searchRow}>
        ...
        >
          <Ionicons name="options" size={24} color={theme.textSecondary} />
        </TouchableOpacity>
      </View>
      ...
      {selectedSubjectIds.size > 0 && (
        <TouchableOpacity
          ...
          onPress={startCustomLesson}
          activeOpacity={0.8}
        >
          <Ionicons name="school" size={24} color="white" />
          <Text style={styles.startButtonText}>
            Start Lessons ({selectedSubjectIds.size} item
                  ...
                  >
                    {selectedListIds.length === 0
                      ...
                      : `${selectedListIds.length} list${
                        ...
                        } selected • ${selectedListSubjectCount} unique subjects.`}
                  </Text>
