...
Web and native playback use the shared publication index and its verified public release origin, `https://zcvoxqcvobgvcwcrqytz.supabase.co`, by default. No environment variable or credential is required to play these published recordings. To deliberately use another host, override `NEXT_PUBLIC_CUSTOM_VOCABULARY_AUDIO_SUPABASE_URL` for web or `EXPO_PUBLIC_CUSTOM_VOCABULARY_AUDIO_SUPABASE_URL` for Expo. Empty or missing overrides use the released origin; invalid nonempty overrides are rejected. Overrides are bundled at build time, so a rebuild/update is required after changing them. Only recordings present in the verified publication index can play; this does not expose local evaluation files. The web playback fixes were [deployed on September 10](releases/custom-vocabulary-web-2026-09-10.md); no native app release was performed as part of that deployment.
