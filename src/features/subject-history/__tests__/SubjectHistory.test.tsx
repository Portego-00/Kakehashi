...
jest.mock('../../../utils/theme', () => ({ useTheme: () => ({ theme: { backgroundColor: '#fff', textColor: '#222', textLight: '#666', border: '#ccc' } }) }));
jest.mock('../../../utils/store', () => ({ useAuthStore: (select: (state: { apiToken: string }) => unknown) => select({ apiToken: 'test-history-token' }) }));
