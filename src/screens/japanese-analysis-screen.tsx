...
} from "../services/urlContentService";
...
type ReaderMode = "url" | "text";

type JapaneseAnalysisScreenProps = {
...
const PANEL_ENTERING = FadeInDown.duration(220).easing(
  Easing.out(Easing.cubic)
);
const PANEL_EXITING = FadeOutUp.duration(150).easing(Easing.in(Easing.quad));
const PANEL_LAYOUT = LinearTransition.duration(220).easing(
  Easing.inOut(Easing.quad)
);
const TRANSLATION_ENTERING = FadeInDown.duration(180).easing(
  Easing.out(Easing.cubic)
);
const TRANSLATION_EXITING = FadeOut.duration(120);
...
export default function JapaneseAnalysisScreen({
  readerMode = "url",
}: JapaneseAnalysisScreenProps) {
  ...
  const refreshJpdbKeyState = useCallback(async () => {
    try {
      ...
      setHasStoredJpdbApiKey(hasStoredKey);
      if (readerMode === "text" && hasStoredKey) {
        setStudyMode("full");
      }
    } catch {
  ...
  const isTextReader = readerMode === "text";
  const isShowingTextAnalysis =
    isTextReader && contentKind === "text" && blocks.length > 0;
  const translationMenuLabel = isTranslating
    ? "Translating…"
    : translationError
      ? "Retry translation"
  ...
  const translationMenuSystemImage = isTranslating
    ? "hourglass"
    : translationError
      ? "arrow.clockwise"
      : translation && showTranslation
  ...
  }, [combinedText, allSubjects, studyMode, analysisRevision]);
  ...
  const resetTextTranslation = useCallback(() => {
  ...
  const handleAnalyzeText = useCallback((textOverride?: string) => {
    const trimmedText = (textOverride ?? textInput).trim().slice(0, 5000);
    if (!trimmedText) {
      setError("Paste or enter some Japanese text to analyze.");
      return;
    }
    ...
    setKanjiMatches([]);
    setJpdbParsedTokens([]);
    setBlocks([{ type: "paragraph", content: trimmedText }]);
    setAnalysisRevision((revision) => revision + 1);
    requestAnimationFrame(() => {
      contentScrollRef.current?.scrollTo({ y: 0, animated: true });
    });
  }, [handleCloseTooltip, resetTextTranslation, textInput]);

  const handleStartNewText = useCallback(() => {
    Keyboard.dismiss();
    handleCloseTooltip();
    resetTextTranslation();
    setTextInput("");
  ...
  }, [handleCloseTooltip, resetTextTranslation]);

  const handleToggleTranslation = useCallback(async () => {
    if (isTranslating || !combinedText.trim()) {
      return;
    }
    ...
    try {
      const translatedText = (
        await azureTranslatorService.translate(combinedText, "ja", "en")
      ).trim();
      ...
      console.error("Failed to translate analyzed text:", translationFailure);
      if (translationRequestIdRef.current === requestId) {
        setTranslationError("Could not translate this text. Please try again.");
      }
    } finally {
      if (translationRequestIdRef.current === requestId) {
        setIsTranslating(false);
  ...
  }, [combinedText, isTranslating, translation]);

  const handleTranslationMenuPress = useCallback(() => {
    setShowStudyModeMenu(false);
  ...
  const handleTextInputChange = useCallback(
    ...
    [blocks.length, error, handleCloseTooltip]
  );
  ...
  const handlePasteText = useCallback(async () => {
    try {
      ...
      const textToAnalyze = clipboardText.slice(0, 5000);
      setTextInput(textToAnalyze);
      handleAnalyzeText(textToAnalyze);
    } catch (clipboardError) {
      console.error("Failed to read clipboard:", clipboardError);
      setError("Could not read clipboard.");
    }
  ...
  }, [handleLoadUrl, isTextReader, sharedUrlParam]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundColor }]}>
      ...
      >
        <View
          style={[
            ...
            isShowingTextAnalysis && styles.headerActionsExpanded,
          ]}
        >
          <GlassButton
            iconName="arrow-back"
            iconColor={headerIconColor}
            style={styles.headerButtonBase}
        ...
        <View style={styles.headerCenter}>
          <Text
            ...
            numberOfLines={1}
          >
            {isTextReader ? "Analyze Text" : "URL Reader"}
          </Text>
        </View>
        <View
          style={[
            styles.headerActions,
            styles.headerTrailingActions,
            isShowingTextAnalysis && styles.headerActionsExpanded,
          ]}
        >
          {isShowingTextAnalysis ? (
            <Animated.View
          ...
          {Platform.OS === "ios" && SwiftUI ? (
            <SwiftUI.Host matchContents style={styles.headerActionHost}>
              ...
              >
                ...
                />
                {isShowingTextAnalysis ? <SwiftUI.Divider /> : null}
                {isShowingTextAnalysis ? (
                  <SwiftUI.Button
                    label={translationMenuLabel}
                    systemImage={translationMenuSystemImage}
                    onPress={handleTranslationMenuPress}
                  />
                ) : null}
              </SwiftUI.Menu>
            </SwiftUI.Host>
      ...
      >
        {isTextReader && !isShowingTextAnalysis ? (
          <Animated.View
          ...
          >
            <TextInput
              ...
              style={[
                styles.japaneseTextInput,
                fontStyles.japaneseText,
                { color: theme.textColor },
              ]}
            />
            <View style={[styles.textInputFooter, { borderTopColor: theme.border }]}>
              ...
              <View style={styles.textInputActions}>
                <TouchableOpacity
                  style={[
                    ...
                    styles.pasteTextButton,
                    { borderColor: theme.border },
                  ]}
                  onPress={() => void handlePasteText()}
                  activeOpacity={0.75}
                  accessibilityRole="button"
                  accessibilityLabel="Paste Japanese text and analyze"
                >
                  <Ionicons name="clipboard-outline" size={17} color={theme.textColor} />
                  <Text style={[styles.textActionLabel, { color: theme.textColor }]}>Paste</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.textActionButton,
                    { backgroundColor: theme.primary },
                    (!textInput.trim() || isAnalyzing) && styles.textActionButtonDisabled,
                  ]}
                  onPress={() => handleAnalyzeText()}
                  activeOpacity={0.8}
                  disabled={!textInput.trim() || isAnalyzing}
                  accessibilityRole="button"
                  accessibilityLabel="Analyze Japanese text"
                >
                  {isAnalyzing ? (
                  ...
                  <Text style={[styles.textActionLabel, styles.analyzeTextLabel]}>Analyze</Text>
                </TouchableOpacity>
              </View>
            </View>
          </Animated.View>
        ...
        {!isLoading && !error && blocks.length === 0 ? (
          ...
          >
            <Ionicons
              name={isTextReader ? "document-text-outline" : "globe-outline"}
              size={24}
              color={theme.textSecondary}
            />
            <Text style={[styles.stateText, { color: theme.textSecondary }]}>
              {isTextReader
                ? "Paste Japanese text to identify grammar, verbs, vocabulary, and kanji."
        ...
        {blocks.length > 0 ? (
          ...
          >
            ...
            {isShowingTextAnalysis && showTranslation ? (
              <Animated.View
                entering={TRANSLATION_ENTERING}
                exiting={TRANSLATION_EXITING}
                layout={PANEL_LAYOUT}
                style={[
                  ...
                  {isShowingTextAnalysis ? (
                    <>
                      ...
                      <TouchableOpacity
                        style={[
                          styles.settingsModeOption,
                          isTranslating && styles.settingsModeOptionDisabled,
                        ]}
                        onPress={handleTranslationMenuPress}
                        activeOpacity={0.8}
                        disabled={isTranslating}
                        accessibilityRole="button"
                        accessibilityLabel={translationMenuLabel}
                      >
                        <Text
                          style={[
                            styles.settingsModeLabel,
