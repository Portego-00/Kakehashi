...
export async function bunproIdentity(sealed?: string) {
  ...
  if (!identity) throw new BunproError("Sign in to continue.", 401);
  return identity;
}
...
export async function bunproRequest<T>(token: string, path: string, body?: unknown, method?: "PATCH", timeoutMs = 20_000): Promise<T> {
