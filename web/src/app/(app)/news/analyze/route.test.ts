...
describe("JPDB news analysis route", () => {
  ...
  it("normalizes JPDB tuples without exposing the API key", async () => {
    ...
    expect(payload).toEqual({ provider: "jpdb", tokens: [
      ...
      { start: 2, end: 3, surface: "へ", spelling: "へ", reading: "へ", meaning: "to", meanings: ["to"], alternativeSpellings: [], partsOfSpeech: ["prt"], tokenType: "grammar" },
    ] });
    ...
    expect(JSON.parse(String(remote.mock.calls[0]?.[1]?.body)).vocabulary_fields).toContain("alt_spellings");
