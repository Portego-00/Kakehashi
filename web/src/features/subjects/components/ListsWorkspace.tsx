...
export function ListsWorkspace() {
  ...
  const visibleSubjects = useMemo(() => {
    ...
    const items = active.subjectIds.map((id) => subjectById.get(id)).filter((subject): subject is Subject => Boolean(subject)).filter((subject) => matchesVocabularyTypes(subject, vocabularyTypes)).filter((subject) => !normalized || subject.data.characters?.includes(normalized) || subject.data.meanings.some((meaning) => meaning.meaning.toLocaleLowerCase().includes(normalized)));
    ...
    return items;
  }, [active, filter, sort, subjectById, vocabularyTypes]);
  const create = () => {
