...
export function SearchWorkspace({ initialState = DEFAULT_SEARCH_STATE }: { initialState?: SearchState }) {
  ...
  const [vocabularyTypes, setVocabularyTypes] = useState<string[]>(initialState.vocabularyTypes);
  ...
  const [showFilters, setShowFilters] = useState(initialState.types.length + initialState.vocabularyTypes.length + initialState.srs.length > 0 || initialState.minLevel > 1 || initialState.maxLevel < 60);
  ...
  const results = useMemo(() => searchSubjects(subjects, assignments, { ...DEFAULT_SEARCH_FILTERS, query: deferredQuery, types, vocabularyTypes, srs, minLevel, maxLevel }), [assignments, deferredQuery, maxLevel, minLevel, srs, subjects, types, vocabularyTypes]);
  ...
  const currentSearchHref = useMemo(() => searchHref({ query, types, vocabularyTypes, srs, minLevel, maxLevel, visiblePages }), [maxLevel, minLevel, query, srs, types, vocabularyTypes, visiblePages]);
  ...
  const reset = () => { setTypes([]); setVocabularyTypes([]); setSrs([]); setMinLevel(1); setMaxLevel(60); resetVisiblePages(); };
  ...
  return <main className={`page ${styles.page}`}>
    ...
    {showFilters ? <Card id="subject-search-filters" className={styles.filters} padding="none">
      ...
      <div className={styles.filterGrid}>
        <div className={styles.filterGroup} role="group" aria-labelledby="subject-type-filter-title"><h2 id="subject-type-filter-title" className={styles.filterTitle}>Subject type</h2><div className={styles.checkGroup}>{TYPES.map((type) => <label key={type.value}><input type="checkbox" checked={types.includes(type.value)} onChange={() => toggle(type.value, types, setTypes)} /><span>{type.label}</span></label>)}</div><VocabularyTypeFilter subjects={subjects} selected={vocabularyTypes} onChange={(next) => { setVocabularyTypes(next); resetVisiblePages(); }} /></div>
