...
export function KanjiProgressExplorer() {
  ...
  const rows = useMemo(() => {
    ...
    return subjects
      ...
      .sort((a, b) => b.stage - a.stage || a.subject.data.level - b.subject.data.level || a.subject.id - b.subject.id);
  ...
  return <main className={`page ${styles.page}`} {...firstReveal}>
    ...
    <Card className={styles.filterPanel}>
      ...
      <div className={styles.filterGrid}>
        ...
        <label><span>SRS stage</span><select value={srs} onChange={(event) => setSrs(event.target.value)}><option value="all">All stages</option>{SRS_LEGEND.map(({ value, label }) => <option value={value} key={value}>{label}</option>)}</select></label>
      </div>
    </Card>

    {rows.length > 0 ? <section><div className={styles.gridHead}><div><h2>{CATALOG_LABELS[catalog]} kanji</h2><p>Sorted from burned to not started. Colors show each kanji&apos;s live SRS stage.</p></div><ul className={styles.gridLegend} aria-label="Kanji SRS color legend">{SRS_LEGEND.map(({ value, label }) => <li key={value}><i data-srs={value} aria-hidden />{label}</li>)}</ul></div><div className={styles.kanjiGrid}>{rows.map(({ subject, stage }) => <Link key={subject.id} href={`/subjects/${subject.id}`} className={styles.kanjiCell} data-srs={srsBucketForStage(stage).toLowerCase()} title={`${subject.data.characters}: ${subject.data.meanings.find((meaning) => meaning.primary)?.meaning ?? subject.data.slug}; Level ${subject.data.level}; ${srsBucketForStage(stage)}`}><span>{subject.data.characters}</span><small>{subject.data.level}</small></Link>)}</div></section> : <EmptyState icon={<Grid3X3 />} title="No kanji match" description="Broaden the catalog, level, or SRS filter." />}
  </main>;
}
