...
export function LevelProjectionChart({ curves, selected, level, goal, now, targetDate }: {
  curves: ProjectionCurve[]; selected: ProjectionScenario["key"]; level: number; goal: number; now: Date; targetDate: string;
}) {
  const reduced = useReducedMotion();
  const [touch, setTouch] = useState(false);
  const data = useMemo(() => projectionTimeline(curves), [curves]);
  const start = now.getTime();
  const end = Math.max(start + 86_400_000, ...curves.map((curve) => curve.points.at(-1)!.timestamp));
  const target = targetDate ? new Date(`${targetDate}T23:59:59`).getTime() : NaN;
  const ticks = [start, start + (end - start) / 2, end];
  const levelTicks = [...new Set([level, ...[10, 20, 30, 40, 50, 60].filter((tick) => tick > level && tick < goal), goal])];
  const formatTick = (value: number) => value === start ? "Today" : new Date(value).toLocaleDateString(undefined, end - start < 90 * 86_400_000 ? { month: "short", day: "numeric" } : { month: "short", year: "2-digit" });
  return <div className={styles.chart} role="group" aria-label="Level projection scenarios" data-chart-kind="line" data-motion={reduced ? "reduced" : "animated"} onPointerDownCapture={(event) => setTouch(event.pointerType === "touch")}>
    <div className={styles.plot} data-chart-plot>
      {curves.length ? <div className={styles.canvas}><ResponsiveContainer width="100%" height="100%" minWidth={0} initialDimension={{ width: 400, height: 240 }}>
        <ComposedChart data={data} margin={{ top: 16, right: 18, bottom: 0, left: 0 }} accessibilityLayer title="Level projection scenarios" aria-label="Level projection scenarios">
          ...
          <YAxis domain={[level, goal]} ticks={levelTicks} tickFormatter={(value: number) => `L${value}`} tick={{ fill: "var(--color-muted)", fontSize: 11 }} tickLine={false} axisLine={false} width={34} allowDecimals={false} />
