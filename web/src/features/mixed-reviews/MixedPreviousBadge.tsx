...
export function MixedPreviousBadge({ answer, animate }: { answer?: MixedPreviousAnswer | null; animate: boolean }) {
  ...
  const props = { className: styles.previousSubjectLink, "data-animate": animate || undefined, "data-correct": answer.correct, "aria-label": `Previous ${answer.source === "bunpro" ? "Bunpro" : "WaniKani"} answer: ${answer.title}, ${answer.correct ? "correct" : "incorrect"}` };
  ...
  return answer.subject ? <Link {...props} href={`/subjects/${answer.subject.id}`}>{content}</Link> : <span {...props}>{content}</span>;
}
