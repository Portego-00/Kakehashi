const stages = ["Beginner 1", "Beginner 2", "Beginner 3", "Adept 1", "Adept 2", "Seasoned 1", "Seasoned 2", "Expert 1", "Expert 2", "Master"];
...
function sources(value: unknown) {
  ...
  const review = record(root.review ?? root.updated_review);
  return [root, data, record(data.attributes), review, record(review.attributes), record(record(review.data).attributes)];
}
export function bunproStage(value: unknown) {
  ...
  return { number, label: label || (number === undefined ? "" : stages[number - 1] ?? `Stage ${number}`) };
}
