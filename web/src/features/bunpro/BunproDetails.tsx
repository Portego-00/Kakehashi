...
export function BunproDetails({ kind, slug, content }: { kind: "grammar" | "vocab"; slug: string; content?: BunproLearnContentItem }) {
  const id = useId();
  ...
  return <section className={styles.details} aria-label="Bunpro item details">
    ...
    <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${tab}`} className={styles.detailContent}><ExampleAudioProvider key={tab}>
      {tab === "Details" ? <>
