...
export function CustomVocabularyWidget({ scope, username = String(scope) }: { scope: string | number; username?: string }) {
  ...
  const packs = customSrs.packs ?? CUSTOM_VOCABULARY_PACKS;
  ...
  const dashboardData = useMemo(() => ({
    lessons: customLessonWords(customSrs.state, packs).length,
    reviews: customReviewWords(customSrs.state, packs, now).length,
    forecast: createReviewForecast(customReviewForecastEntries(customSrs.state, packs), now),
  }), [customSrs.state, now, packs]);
