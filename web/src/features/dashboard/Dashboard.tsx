...
import { assignmentsQuery, availableReviewCountQuery, levelProgressionsQuery, reviewStatisticsQuery, subjectsQuery, userQuery } from "@/lib/wanikani/queries";
import { waniKaniUserId } from "@/lib/wanikani/user-identity";
import type { Subject } from "@/types/wanikani";
import { assignmentActivityDays, burnedSubjectRows, criticalSubjectRows, incompleteLevelRows, isLessonAvailable, isReviewAvailable, levelProgress, levelWidgetSubjects, recentMistakeRows, recentUnlockRows, srsStageSpread, todayStudyActivity, type DashboardSubjectRow } from "./dashboard-data";
...
import { BunproForecast } from "@/features/bunpro/BunproForecast";
...
const SUBJECT_CATALOG_SECTIONS = new Set(["forecast", "recent-mistakes", "subject-lists", "incomplete-levels", "recent-unlocks", "critical-items", "burned-items"]);
...
export function Dashboard() {
  ...
  const forecast = useMemo(() => createReviewForecast(wanikaniReviewForecastEntries(assignments.data ?? [], allSubjects.data ?? [], currentLevel), now), [assignments.data, allSubjects.data, currentLevel, now]);
  ...
  const sections: Record<string, React.ReactNode> = {
    "daily-study": <section className={`${styles.section} ${styles.queueSection}`} aria-label="Daily study">{currentVacationStartedAt ? <VacationNotice startedAt={currentVacationStartedAt} refresh={currentUser.refetch} /> : <><SectionHeader title="Today" detail={isDemo ? "Your demo study queues" : "Your live WaniKani queues"}>{!isDemo ? <VacationModeControls active={false} refresh={currentUser.refetch} showRefresh={false} className={styles.vacationHeaderAction} /> : null}</SectionHeader><div className={styles.queue}><StudyQueueCard available={canAccessCoreStudy(username)} demo={isDemo} type="lesson" count={lessonCount} loading={availabilityLoading} /><StudyQueueCard available={canAccessCoreStudy(username)} demo={isDemo} type="review" count={reviewCount} loading={availabilityLoading} /></div></>}<BunproHomeButton wanikaniCount={availabilityLoading || (availableReviewCount.error && !assignments.data) ? undefined : reviewCount} /></section>,
    ...
    forecast: <section className={styles.section}>{currentVacationStartedAt ? <><SectionHeader title="Review forecast" detail="Review scheduling is paused" /><VacationNotice startedAt={currentVacationStartedAt} compact /></> : <BunproForecast entries={wanikaniReviewForecastEntries(assignments.data ?? [], allSubjects.data ?? [], currentLevel)} forecast={forecast} {...forecastPreferences} loading={assignments.isLoading || allSubjects.isLoading} unavailable={(assignments.error && !assignments.data) || (allSubjects.error && !allSubjects.data) ? "WaniKani review schedule is unavailable." : undefined} />}</section>,
