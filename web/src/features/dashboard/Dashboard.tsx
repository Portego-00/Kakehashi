...
export function Dashboard() {
  ...
  const sections: Record<string, React.ReactNode> = {
    ...
    forecast: <section className={styles.section}>{currentVacationStartedAt ? <><SectionHeader title="Review forecast" detail="Review scheduling is paused" /><VacationNotice startedAt={currentVacationStartedAt} compact /></> : <ReviewForecast forecast={forecast} {...forecastPreferences} loading={assignments.isLoading || allSubjects.isLoading} unavailable={(assignments.error && !assignments.data) || (allSubjects.error && !allSubjects.data) ? "WaniKani review schedule is unavailable." : undefined} />}</section>,
