...
describe("native-parity study configuration", () => {
  it("shows crossword presets and never exposes the generic 100-item slider", () => {
    ...
    render(<StudyConfig mode="crossword" filters={getModeDefaultFilters("crossword", 60)} subjects={[]} lists={[]} onChange={onChange} onStart={vi.fn()} />);
  ...
  it("matches the native listening and context controls", () => {
    const { rerender } = render(<StudyConfig mode="listening" filters={{ ...getModeDefaultFilters("listening", 60), animeSources: ["death_note"] }} subjects={[]} lists={[]} onChange={vi.fn()} onStart={vi.fn()} />);

    expect(screen.getByPlaceholderText("death_note, your_name")).toBeInTheDocument();
    ...
    rerender(<StudyConfig mode="context-sentences" filters={getModeDefaultFilters("context-sentences", 60)} subjects={[]} lists={[]} onChange={vi.fn()} onStart={vi.fn()} />);
