...
import { matchesVocabularyTypes } from "@/features/subjects/vocabulary-types";
...
export function SubjectLists({ subjects, scope, username }: { subjects: Subject[]; scope: StudyStorageScope; username: string }) {
  ...
  const shown = useMemo(() => subjects.filter((subject) => matchesVocabularyTypes(subject, vocabularyTypes)).filter((subject) => !query.trim() || subject.data.characters?.includes(query) || subject.data.meanings.some((item) => item.meaning.toLocaleLowerCase().includes(query.toLocaleLowerCase()))).slice(0, 100), [query, subjects, vocabularyTypes]);
  useEffect(() => {
