...
describe("community server boundary", () => {
  it("uses only a development/test fallback when durable configuration is absent", () => {
    expect(resolveCommunityMode({ url: "", serviceRoleKey: "", production: false })).toBe("local-server");
