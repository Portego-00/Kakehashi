...
export function SettingsWorkspace() {
  ...
  return <main className={`page ${styles.page}`}>
    ...
    <section data-settings-search="" className={styles.settingsSection} aria-labelledby="support-heading">
    ...
    <section data-settings-search="" className={styles.settingsSection} aria-labelledby="appearance-heading" data-search-keywords="theme system light dark midnight sepia">
    ...
    <section data-settings-search="" className={styles.settingsSection} aria-labelledby="text-heading" data-search-keywords="font scale size compact standard large accessibility">
    ...
    <section data-settings-search="" className={styles.settingsSection} aria-labelledby="colors-heading" data-search-keywords="radical kanji vocabulary palette color colour">
    ...
    <section className={styles.settingsSection} aria-labelledby="reviews-heading">
      ...
      <Card padding="none" className={styles.preferenceCard}>
        ...
        {settings.study.reviewTypeOrderEnabled ? REVIEW_TYPE_POSITIONS.map((positionLabel, position) => {
          ...
          return <label data-settings-search="" className={styles.selectRow} key={positionLabel}><span><strong>{positionLabel} item type</strong><small>{positionLabel} group shown when item type grouping is enabled.</small></span><select aria-label={`${positionLabel} item type`} value={reviewType} onChange={(event) => updateReviewTypePosition(position, event.target.value as WebSettings["study"]["reviewTypeOrder"][number])}>{Object.entries(REVIEW_TYPE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>;
        }) : null}
        ...
        <ToggleRow label="Pause on close answer" description="Wait after a fuzzy meaning match so you can inspect what was accepted." checked={settings.study.pauseOnCorrect || settings.study.pauseOnClose} disabled={settings.study.pauseOnCorrect} onChange={(value) => updateStudy("pauseOnClose", value)} />
        ...
        {settings.study.ankiMode !== "off" ? <>
          ...
          <StudyShortcutSettings value={settings.study.studyShortcuts} onChange={(value) => updateStudy("studyShortcuts", value)} />
        ...
        <div data-settings-search="Anki export" data-search-keywords="AnkiConnect desktop sentences translations"><AnkiExportSettingsButton /></div>
        ...
        <ToggleRow label="Autoplay vocabulary audio" keywords="voice actor pronunciation Kyoko Kenichi female male" description="Play pronunciation after a correct reading or when an Anki reading is revealed." checked={settings.study.autoplayAudio} onChange={(value) => updateStudy("autoplayAudio", value)} />
...
function DashboardLayoutEditor({ settings, onChange, customSrsAccessible }: { settings: WebSettings; onChange: (settings: WebSettings) => void; customSrsAccessible: boolean }) {
  ...
  return <Card data-settings-search="Dashboard layout" padding="none" className={`${styles.preferenceCard} ${styles.dashboardEditor}`}>
...
export function JapaneseVoiceDownloadSetting() {
  ...
  return <section data-settings-search="" id="japanese-voice" className={`${styles.settingsSection} ${styles.settingsAnchor}`} aria-labelledby="japanese-voice-heading">
...
function ToggleRow({ label, description, checked, onChange, icon, disabled = false, keywords = "" }: { label: string; description: string; checked: boolean; onChange: (value: boolean) => void; icon?: React.ReactNode; disabled?: boolean; keywords?: string }) {
  return <label data-settings-search="" data-search-keywords={keywords} className={styles.toggleRow} aria-disabled={disabled}><span>{icon}<span><strong>{label}</strong>{description ? <small>{description}</small> : null}</span></span><input type="checkbox" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} /><i aria-hidden /></label>;
}
